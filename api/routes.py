...
def list_doctors(
    ...
    limit: int = Query(100, ge=1, le=500, description="Max number of doctors to return"),
):
    """
    ...
    Returns:
        List of doctors
    """
    logger.info(f"Listing doctors (specialty={specialty or 'all'}, limit={limit})")

    try:
        settings = get_settings()
        db_session = next(get_db_session(settings))
        doctors = (
            get_doctors_by_specialty(db_session, specialty)
            if specialty
            else get_all_doctors(db_session)
        )
